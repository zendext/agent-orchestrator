...
describe("loadBuiltins", () => {
  ...
  it("registers the local tracker from importFn", async () => {
    const registry = createPluginRegistry();
    const fakeTracker = makePlugin("tracker", "local");

    await registry.loadBuiltins(undefined, async (pkg: string) => {
      if (pkg === "@aoagents/ao-plugin-tracker-local") return fakeTracker;
      throw new Error(`Not found: ${pkg}`);
    });

    expect(registry.list("tracker")).toContainEqual(
      expect.objectContaining({ name: "local", slot: "tracker" }),
    );
  });
